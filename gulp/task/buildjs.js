    ...
    module.exports.compileTpl = function(file){
        ...
        var src_path = file || pathCommon.allFile(config.paths.baseJs,"hbs");
    ...
    module.exports.vendor = function(){
        ...
        gulp.src(src_path, {base: config.paths.baseJs})
    ...
    module.exports.js = function(){
        ...
        var amdConfig = {
            'baseUrl': path.join(config.paths.cur,config.paths.baseJs),
        ...
        glob(pathCommon.allFile(config.paths.srcMainJs,'js'),function(err,files){
            while(files.length){
                ...
                newName = name.replace(/\/|\\/ig, '_').replace('www_js_main_', '');
                gulp.src(file)
                    ...
                    .pipe(gulp.dest(config.paths.distJs))
                    .pipe(gulpif(!global.isDev,rev.manifest(config.paths.assetsJs,{
                        merge: true
                    })))
